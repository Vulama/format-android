...
class EditGroupViewModel(
    ...
    fun addLocalGroup(groupName: String, formulas: List<FormulaEntry>) {
        ...
        val encodedFormulaGroup = FormulaGroup(
            name = groupName,
            formulas = formulas.map { it.copy(formula = URLEncoder.encode(it.formula, "UTF-8")) },
        )
        formulaStore.setLocal(localGroups + encodedFormulaGroup)
    ...
    fun updateGroup(groupName: String, formulas: List<FormulaEntry>, oldFormulaGroup: FormulaGroup?) {
        ...
        val localGroups = formulaStore.getLocal()
        val updatedFormulaGroup = FormulaGroup(
            name = groupName,
            formulas = formulas.map { it.copy(formula = URLEncoder.encode(it.formula, "UTF-8")) },
            isFavourite = oldFormulaGroup?.isFavourite ?: false
        )
